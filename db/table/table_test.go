...
func Testテーブルとしての等価性の判定(t *testing.T) {
	...
	t.Run("カラムの値が不一致", func(t *testing.T) {
		table1 := BuildTable().WithName("name").WithRow(NewColumn("c1", 1), NewColumn("c2", 2)).Build()
		table2 := BuildTable().WithName("name").WithRow(NewColumn("c1", 100), NewColumn("c2", 2)).Build()
		assert.False(t, table1.IsSameAsTable(table2))
	})

	t.Run("カラムの数が多い", func(t *testing.T) {
		table1 := BuildTable().WithName("name").WithRow(NewColumn("c1", 1)).Build()
		table2 := BuildTable().WithName("name").WithRow(NewColumn("c1", 1), NewColumn("c2", 2)).Build()
		assert.False(t, table1.IsSameAsTable(table2))
	})

	t.Run("カラムの数が少ない", func(t *testing.T) {
		table1 := BuildTable().WithName("name").WithRow(NewColumn("c1", 1), NewColumn("c2", 2)).Build()
		table2 := BuildTable().WithName("name").WithRow(NewColumn("c1", 1)).Build()
		assert.False(t, table1.IsSameAsTable(table2))
	})

	t.Run("カラム名が不一致", func(t *testing.T) {
		table1 := BuildTable().WithName("name").WithRow(NewColumn("c1", 1), NewColumn("c2", 2)).Build()
		table2 := BuildTable().WithName("name").WithRow(NewColumn("cA", 1), NewColumn("c2", 2)).Build()
		assert.False(t, table1.IsSameAsTable(table2))
	})
...
func Test行によるカラムの補完(t *testing.T) {
	t.Run("テーブルに存在していないカラムを行から補完する", func(t *testing.T) {
		table := BuildTable().WithName("name").
